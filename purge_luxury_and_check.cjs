...
const luxuryKeywords = ['lamborghini', 'ferrari', 'porsche', 'bugatti', 'bentley', 'maserati', 'rolls royce', 'mclaren', 'aston martin', 'maybach', 'supercar', 'hypercar', 'rolex', 'patek', 'audemars', 'omega', 'breitling', 'cartier', 'hublot', 'tag heuer', 'gucci', 'vuitton', 'hermes', 'hermès', 'prada', 'chanel', 'dior', 'balenciaga', 'versace', 'givenchy', 'yves saint', 'armani', 'burberry', 'tiffany', 'boucheron', 'bulgari', 'yacht', 'private jet', 'jet prive', 'mansion', 'penthouse', 'villa', 'chateau', 'luxury', 'luxe', 'prestige', 'platinum', 'diamond', 'emerald', 'ruby', 'sapphire', 'gold bar', 'rare edition', 'limited prestige', 'exclusive mansion', 'luxury estate'];

let cleaned = db.filter(item => {
  const name = (item.productName || item.title || '').toLowerCase();
  const src = (item.source || '').toLowerCase();
  const desc = (item.reviewText || '').toLowerCase();
  const fullText = `${name} ${src} ${desc} ${item.description || ''}`.toLowerCase();
  
  const hasLuxuryKeyword = luxuryKeywords.some(kw => fullText.includes(kw));
  const isTooExpensive = item.price > 5000; // Baissé de 8000 à 5000 pour être plus strict
...
async function runBatch(items) {
  return Promise.all(items.map(item => {
    ...
    return checkImage(img).then(r => ({ ...r, item }));
  }));
}
...
async function main() {
  ...
  const shuffledSolo = [...finalGoodItems].sort(() => Math.random() - 0.5).slice(0, 800);
  fs.writeFileSync('src/data/questions-solo.json', JSON.stringify(shuffledSolo, null, 2));
