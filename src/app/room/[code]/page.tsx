        ...
        <div className="main-content">
            ...
            <div className="card" style={{ textAlign: "center", padding: "3rem 1rem" }}>
              ...
              {room.isLoadingQuestions ? (
                <div style={{ marginTop: "2rem", color: "var(--primary)" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "10px" }}>
                    ...
                    <span>RÃ©cupÃ©ration de produits inÃ©dits...</span>
                  </div>
                  <p style={{ fontSize: "0.8rem", color: "var(--text-muted)", marginTop: "0.5rem" }}>
                    On fouille les bases de donnÃ©es pour toi !
                  </p>
                </div>
              ) : (
                <>
              ...
              <div style={{ marginTop: "2rem" }}>
                {isHost ? (
                  ...
                  >
                    <Play size={20} style={{ marginRight: "8px" }} /> DÃ©marrer la partie
                  </button>
                ) : (
                  <div className="card" style={{ background: "var(--bg-card)", borderStyle: "dashed" }}>
                    <p>{room.isLoadingQuestions ? "PrÃ©paration de la partie..." : "Attente de l'hote..."}</p>
                  </div>
                )}
              </div>
            </div>
                ...
                {!myAnswer ? (
                  <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
                    {(mode === "note" || mode === "both") && (
                      <div className="input-group">
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.5rem" }}>
                          <label style={{ fontWeight: "bold" }}>Note estimÃ©e ?</label>
                    ...
                    {(mode === "price" || mode === "both") && (
                      <div className="input-group">
                        <label style={{ fontWeight: "bold", display: "block", marginBottom: "0.5rem" }}>Prix estimÃ© ? (E)</label>
                    ...
                    <button className="btn btn-primary btn-lg" onClick={() => handleSubmit(false)} style={{ height: "60px" }}>Valider ma rÃ©ponse</button>
                  </div>
                ) : (
                  <div style={{ textAlign: "center", padding: "3rem 1rem", background: "rgba(34, 197, 94, 0.05)", borderRadius: "16px", border: "2px dashed var(--success)" }}>
                    <h3 style={{ color: "var(--success)", fontSize: "1.5rem", marginBottom: "0.5rem" }}>RÃ©ponse enregistrÃ©e !</h3>
          ...
          {room.state === "REVEAL" && currentQ && (
            <div className="card animate-scale-in">
              <h2 style={{ textAlign: "center", fontSize: "1.8rem", marginBottom: "2rem" }}>RÃ©sultats du tour</h2>
              <div className="grid" style={{ gap: "1rem", marginBottom: "2.5rem" }}>
                {(mode === "note" || mode === "both") && (
                  <div className="card" style={{ textAlign: "center", background: "var(--bg-card)" }}>
                    <p style={{ fontSize: "0.9rem", color: "#9ca3af" }}>Note rÃ©elle</p>
                    <p style={{ fontSize: "2rem", fontWeight: "bold", color: "var(--primary)" }}>{currentQ.realRating} â­</p>
                  </div>
                )}
                {(mode === "price" || mode === "both") && (
                  <div className="card" style={{ textAlign: "center", background: "var(--bg-card)" }}>
                    <p style={{ fontSize: "0.9rem", color: "#9ca3af" }}>Prix rÃ©el</p>
                    <p style={{ fontSize: "2rem", fontWeight: "bold", color: "var(--primary)" }}>{currentQ.price} E</p>
                  </div>
                )}
              </div>
                  ...
                  return (
                    <div key={p.id} className="player-result-row" style={{ padding: "1rem", border: "1px solid var(--border)", borderRadius: "12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div>
                        ...
                        {ans ? (
                          <div style={{ fontSize: "0.85rem", color: "#9ca3af" }}>{ans.rating?.toFixed(1)}â­ | {ans.price}E</div>
                        ) : <span style={{ fontSize: "0.8rem", color: "var(--danger)" }}>N'a pas rÃ©pondu</span>}
                      </div>
          ...
          {room.state === "FINISHED" && (
            <div className="card animate-fade-in" style={{ textAlign: "center", padding: "4rem 1rem" }}>
              <h1 style={{ fontSize: "2.5rem", marginBottom: "1rem" }}>Partie terminÃ©e !</h1>
